...
def main(args=None):
        ...
        node = rclpy.create_node('single_robot_simple_py')

        DR_init.__dsr__node = node

        try:
                from DSR_ROBOT2 import (DR_AXIS_X, DR_AXIS_Z, DR_BASE, DR_CIRCLE, DR_LINE,
                                        DR_MV_MOD_ABS, DR_TOOL, ROBOT_MODE_AUTONOMOUS,
                                        move_periodic, move_spiral, moveb, movec, movej, movejx,
                                        movel, movesj, movesx, posb, posj, posx, print_ext_result,
                                        set_accx, set_robot_mode, set_velx)
